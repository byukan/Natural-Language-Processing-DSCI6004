...
def process_file(name, f):
    ...
    TODO: Change only the internals of this function. The assignment is modify and add lines.
    """

    email_pattern1 = '(\w+)@(\w+).edu'  # TODO: Create your own patterns for email & phone
    email_pattern2 = '([\w]+)\s[at]{2}\s([\w]+)\s[dot]{,3}\s([\w]+)\s[dot]{,3}\s([\w]+)'
    phone_pattern = '(\d{3}|\(\d\d\d\))(\s|-|\.)?(\d{3})-(\d{4})'

    email_pattern = "|".join([email_pattern1, email_pattern2])
