...
psychobabble = [
    ...
    [r'(.*) father (.*)',
     ["Who's your daddy?",
      ...
      "I don't like to give the sob story: growing up in a single-parent home, never knew my father",
      "Good family relations are important."]],
...
def reflect(fragment):
    tokens = fragment.lower().split(" ") # : finish this function
...
def analyze(statement):
    ...
    for reg in psychobabble:
        #for reg in lis[0]:
        match = re.findall(reg[0],statement)
        if match:
            #break
            return random.choice(reg[1]).format(match[0])
...
def handle_command(command, channel):
    ...
    slack_client.api_call("chat.postMessage", channel=channel,
                          text=analyze(command), as_user=True)
