...
class IRSystem:
    ...
    def index(self):
        """
        Build an index of the documents. This is where you will build the inverted index. The documents will have
        already been read in for you at this point, so you will want to look at some of the instance variables in the
        class:
...
=======
        ...
        # inv_index = defaultdict(set)  # create dictionary with sets for the values
        ...
        # uniq = set()
        # for word in self.docs[:1]:
        #     uniq.add(word)

        # print(uniq)


        # print(self.vocab = [xx for xx in self.get_uniq_words()])
        # print(xx for xx in self.get_uniq_words())
        ...
        # for i, x in enumerate(self.docs):  # loop through each doc
        #     doc_vocab = set(x)  # create doc vocab
    ...
    def boolean_retrieve(self, query):
        ...
        # : Implement Boolean retrieval. You will want to use your inverted index that you created in index().
        # XXX: Right now this just returns all the possible documents!
        # docs = []

        # print(self.inv_index[query[0]])
...
def main(args):
    ...
    # irsys.read_data('4_1_ir_I / exercise_information_retrieval / data / RiderHaggard')
    irsys.index()
